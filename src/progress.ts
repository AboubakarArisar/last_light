...
export type ProgressCache = { version: 1; base: Save; save: Save; pending: PendingSave | null };

export function newCache(save = fresh()): ProgressCache {
  return { version: 1, base: fresh(), save: structuredClone(save), pending: null };
}
...
export function readCache(raw: string): ProgressCache {
  ...
  return {
    version: 1, base: readSave(value.base), save: readSave(value.save),
